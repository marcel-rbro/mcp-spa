  ...
  "breathe",
  {
    ...
    inputSchema: {
      seconds: z
        ...
        .optional()
        .describe(`How long to rest, 0–${MAX_REST_SECONDS}s.`),
    },
  },
...
server.registerPrompt(
  ...
  {
    title: "Spa day",
    description: "The full circuit: check in → breathe → sauna → meditation → check out.",
  },
  async () => ({
    messages: [
      {
...
server.registerResource(
  ...
  async (uri) => ({
    contents: [{ uri: uri.href, mimeType: "text/plain", text: checkIn().text }],
  }),
);
...
server.registerResource(
  ...
  new ResourceTemplate("spa://ambient/{sound}", {
    list: async () => ({
      resources: Object.keys(SOUNDSCAPES).map((sound) => ({
        uri: `spa://ambient/${sound}`,
        name: sound,
      })),
    }),
  }),
